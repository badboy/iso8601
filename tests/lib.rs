...
fn test_millisecond() {
    ...
    assert_eq!(Done(&[][..],
               ...
               parse_time(b"16:43:16.123+00:00"));
    assert_eq!(Done(&[][..],
                    Time {
                        hour: 16,
                        minute: 43,
    ...
    assert_eq!(Done(&[][..],
               ...
               parse_time(b"16:43:16.123+05:00"));
}
...
#[test]
fn short_time5() {
    assert_eq!(parse_time(b"164800.1"),
               Done(&[][..],
                    Time {
                        hour: 16,
                        minute: 48,
...
#[test]
fn short_time6() {
    assert_eq!(parse_time(b"164800.1Z"),
               Done(&[][..],
                    Time {
                        hour: 16,
                        minute: 48,
...
#[test]
fn short_time7() {
    assert_eq!(parse_time(b"16:48:00"),
               Done(&[][..],
                    Time {
                        hour: 16,
                        minute: 48,
...
#[test]
fn short_dtim1() {
    assert_eq!(parse_datetime(b"20070831T1648"),
               Done(&[][..],
                    DateTime {
                        date: Date::YMD {
                            year: 2007,
...
#[test]
fn short_dtim2() {
    assert_eq!(parse_datetime(b"20070831T1648Z"),
               Done(&[][..],
                    DateTime {
                        date: Date::YMD {
                            year: 2007,
...
fn test_ordinal_date() {
    ...
    assert_eq!(Done(&[][..],
               ...
               ordinal_date(b"2015-358"));
    assert_eq!(Done(&[][..],
                    Date::Ordinal {
                        year: 2015,
                        ddd: 399,
                    }),
               ordinal_date(b"2015-399"));
    assert_eq!(Done(&[][..],
                    Date::Ordinal {
                        year: 2015,
                        ddd: 000,
                    }),
               ordinal_date(b"2015-000"));
...
fn format_equivalence() {
    assert_eq!(parse_datetime(b"2001-02-03T04:05:06+07:00"),
               parse_datetime(b"20010203T040506+0700"));
    assert_eq!(parse_datetime(b"2001-02-03T04:05:06+07:00"),
               parse_datetime(b"20010203T04:05:06+0700"));
    assert_eq!(parse_datetime(b"2001-02-03T04:05:00+07:00"),
               parse_datetime(b"20010203T0405+0700"));
    assert_eq!(parse_datetime(b"20010203T0405+0700"),
               parse_datetime(b"2001-02-03T0405+0700"));
    assert_eq!(parse_datetime(b"20010203T040506+0700"),
               parse_datetime(b"2001-02-03T040506+0700"));
    assert_eq!(parse_datetime(b"20010203T040506+0000"),
               parse_datetime(b"20010203T040506Z"));
    assert_eq!(parse_datetime(b"2015W056T04:05:06+07:00"),
               parse_datetime(b"2015-W05-6T04:05:06+07:00"));
}

#[test]
fn test_datetime_correct() {
    assert_eq!(parse_datetime(b"20060831T16:44+00:00"),
               Done(&[][..],
                    DateTime {
                        date: Date::YMD {
                            year: 2006,
    ...
    assert_eq!(parse_datetime(b"2007-08-31T16:45+00:00"),
               Done(&[][..],
                    DateTime {
                        date: Date::YMD {
                            year: 2007,
    ...
    assert_eq!(parse_datetime(b"20070831T1646+00:00"),
               Done(&[][..],
                    DateTime {
                        date: Date::YMD {
                            year: 2007,
    ...
    assert_eq!(parse_datetime(b"20070831T1647+0000"),
               Done(&[][..],
                    DateTime {
                        date: Date::YMD {
                            year: 2007,
    ...
    assert_eq!(parse_datetime(b"2009-02-01T09:00:22+05"),
               Done(&[][..],
                    DateTime {
                        date: Date::YMD {
                            year: 2009,
    ...
    assert_eq!(parse_datetime(b"2010-01-01T12:00:00+01:00"),
               Done(&[][..],
                    DateTime {
                        date: Date::YMD {
                            year: 2010,
    ...
    assert_eq!(parse_datetime(b"2011-06-30T18:30:00+02:00"),
               Done(&[][..],
                    DateTime {
                        date: Date::YMD {
                            year: 2011,
    ...
    assert_eq!(parse_datetime(b"2015-06-29T23:07+02:00"),
               Done(&[][..],
                    DateTime {
                        date: Date::YMD {
                            year: 2015,
    ...
    assert_eq!(parse_datetime(b"2015-06-26T16:43:16"),
               Done(&[][..],
                    DateTime {
                        date: Date::YMD {
                            year: 2015,
    ...
    assert_eq!(parse_datetime(b"2015-W05-6T04:05:06+07:00"),
               Done(&[][..],
                    DateTime {
                        date: Date::Week {
                            year: 2015,
    ...
    assert_eq!(parse_datetime(b"2015-056T04:05:06+07:00"),
               Done(&[][..],
                    DateTime {
                        date: Date::Ordinal {
                            year: 2015,
    ...
    assert_eq!(parse_datetime(b"2015056T04:05:06+07:00"),
               Done(&[][..],
                    DateTime {
                        date: Date::Ordinal {
                            year: 2015,
    ...
    assert_eq!(parse_datetime(b"2015-297T16:30:48Z"),
               Done(&[][..],
                    DateTime {
                        date: Date::Ordinal {
                            year: 2015,
    ...
    assert_eq!(parse_datetime(b"2015-W43-6T16:30:48Z"),
               Done(&[][..],
                    DateTime {
                        date: Date::Week {
                            year: 2015,
